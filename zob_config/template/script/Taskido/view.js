...
function getSelectOptions() {
	...
	taskFiles.forEach(function (file) {
		...
		var filePath = getFilename(file); //! 注释了上面内容，添加了这行
...
function getTimeline(tasks) {
	...
	for (i = 0; i < timelineDates.length; i++) {
		...
		if (timelineDates[i] == today) {
			...
			todayContent += "<div class='counter' id='todo'><div class='count'>" + todoCount + "</div><div class='label'>To Do</div></div>" //! 和上一行换了顺序
